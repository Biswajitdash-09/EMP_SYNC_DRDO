...
export const useReportQuickActions = () => {
  ...
  const generateReportContent = (reportType: string, reportParams: ReportParams) => {
    // Transform employees to ensure compatibility with report generators
    const transformEmployeesForReports = (employees: typeof allEmployees): Employee[] => {
    ...
    const baseEmployees = transformEmployeesForReports(allEmployees);
    const filteredEmployees = reportParams.department 
      ? baseEmployees.filter(emp => emp.department === reportParams.department)
      : baseEmployees;
    ...
    const filteredLeaveRequests = reportParams.department 
      ? allLeaveRequests.filter(req => {
          const emp = baseEmployees.find(e => e.name === req.employee);
    ...
    switch (reportType) {
      ...
      case 'Department Overview':
        return generateDepartmentReport(baseEmployees, filteredEmployees, reportParams);
      default:
